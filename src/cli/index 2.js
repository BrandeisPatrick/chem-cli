...
export class ChemCLI {
  ...
  displayResponse(response) {
    // Clear space above chat bar for response
    const terminalHeight = process.stdout.rows || 24;
    process.stdout.write(`\x1B[${terminalHeight - 1};1H`);
  ...
  async handlePrecisionChoice(input) {
    ...
    try {
      ...
      console.log(ASCIIArt.getErrorMessage(`Failed to generate calculation: ${error.message}`));
    } finally {
      this.currentPlanData = null;
    }
  }
